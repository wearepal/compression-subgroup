...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs, reduce_memory=False):
        ...
        if reduce_memory:
            ...
            self.working_dir = training_path
        ...
        with tqdm(total=len(train_dataloader), desc="Epoch {}".format(epoch)) as pbar:
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                for i, (student_rep, teacher_rep) in enumerate(zip(new_student_reps, new_teacher_reps)):
                    ...
                    if i == 0 and not args.embedding:
                        continue
                    elif i > 0 and not args.hidden:
                        continue
                ...
                tr_att_loss += att_loss.item() if not isinstance(att_loss, float) else att_loss
                tr_rep_loss += rep_loss.item() if not isinstance(rep_loss, float) else rep_loss
