...
class DocumentDatabase:
    def __init__(self, reduce_memory=False):
        if reduce_memory:
            ...
            self.document_shelf_filepath = os.path.join(self.temp_dir.name, 'shelf.db')
            sqlite3.connect(self.document_shelf_filepath)
            self.document_shelf = shelve.open(self.document_shelf_filepath, flag='n', protocol=-1)
            self.documents = None
        else:
            self.documents = []
